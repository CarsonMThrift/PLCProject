    ...
    (cases expression form
      [define-exp (name definition)
        (eval-exp 
          ...
          ; (set! global-env (extend-env (list name) (list (eval-exp definition (empty-env) k)) global-env))   
      ]
...
(define eval-exp
  (lambda (exp local-env k)
    (cases expression exp
      ...
      [var-exp (id)
				(apply-k k (apply-env local-env id; look up its value.
  ...
  (lambda (proc-value args k)
    (cases proc-val proc-value
      ...
      [closure (arg-names bodies local-env) 
        (cond 
          [(symbol? arg-names) 
            (eval-bodies bodies (extend-env (list arg-names) (list args) local-env) k)
          ]
          [(list? arg-names) 
            (eval-bodies bodies (extend-env arg-names args local-env) k)
          ]
          [else 
            (eval-bodies 
              bodies 
...
(define rep      ; "read-eval-print" loop.
  (lambda ()
    ...
    (let ([answer (top-level-eval (parse-exp (read)) (id-k))])
